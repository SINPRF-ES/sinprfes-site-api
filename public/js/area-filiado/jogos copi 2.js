...
function renderizarFormulario(container) {
    ...
    if (!document.getElementById('style-jogos')) {
        ...
        s.textContent = `
            ...
            .jogos-container-lista { width: 100%; margin: 0 -24px 0 -24px; padding: 0 0 24px 0; }
            
            .categoria-card { background: #ffffff; color: #333333; border: 1px solid #e0e0e0; border-radius: 10px; padding: 25px; margin-bottom: 25px; box-shadow: 0 4px 10px rgba(0,0,0,0.05); }
            .categoria-titulo { font-size: 1.2rem; color: #2c3e50; border-bottom: 2px solid #f0f0f0; padding-bottom: 10px; margin-bottom: 20px; font-weight: bold; }
            .opcoes-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 15px; }
            
            .destaque-adesao { background: #f0f7ff; color: #004085; border: 1px solid #b8daff; padding: 20px; border-radius: 8px; display: flex; align-items: center; gap: 15px; margin-bottom: 30px; cursor: pointer; font-size: 1.1rem; font-weight: 600; }
            .destaque-adesao input { transform: scale(1.5); cursor: pointer; }

            .categoria-card select, .categoria-card textarea, .categoria-card input[type="text"] { width: 100%; padding: 12px; border: 1px solid #ccc; border-radius: 6px; font-size: 1rem; color: #333; background-color: #fff; font-family: inherit; }
            ...
            .jogos-resumo-card { background: #0b1728; border-radius: 12px; border: 1px solid rgba(255,255,255,0.15); padding: 14px 16px; margin-top: 16px; font-size: 0.9rem; color: #e0e7ff; text-align: left; }
            .jogos-resumo-card strong { color: #f1c40f; }
            .jogos-resumo-card small { color: #a0aec0; }

            /* --- TABELA DE GESTÃO --- */
            .tabela-jogos { width: 100%; border-collapse: collapse; min-width: 820px; background-color: transparent; border: 1px solid var(--amarelo); }
            
            .tabela-jogos th { 
                background: #003366; color: #ffffff; padding: 10px 12px; 
                text-align: center !important; vertical-align: middle !important; 
                font-size: 0.85rem; border-bottom: 2px solid var(--amarelo);
                cursor: pointer; user-select: none; position: relative;
            }
            .tabela-jogos th:hover { background-color: #004080; }
            
            /* Setas de ordenação */
            .tabela-jogos th.sort-asc::after { content: " 🔼"; position: absolute; right: 2px; font-size: 0.7rem; }
            .tabela-jogos th.sort-desc::after { content: " 🔽"; position: absolute; right: 2px; font-size: 0.7rem; }

            .tabela-jogos td { 
                padding: 8px 12px; border-top: 1px solid rgba(241,196,15,0.25); border-right: 1px solid rgba(241,196,15,0.20);
                color: var(--texto-claro); background-color: rgba(0, 0, 0, 0.2); vertical-align: middle !important;
                white-space: normal; word-wrap: break-word; font-size: 0.85rem;
            }
            .tabela-jogos th:last-child, .tabela-jogos td:last-child { border-right: none; }
            ...
            .tabela-jogos th:nth-child(1), .tabela-jogos td:nth-child(1) { white-space: nowrap; width: 1%; text-align: left; } /* Nome */
            .tabela-jogos th:nth-child(2), .tabela-jogos td:nth-child(2) { text-align: center; width: 60px; } /* Idade */
            .tabela-jogos th:nth-child(3), .tabela-jogos td:nth-child(3) { text-align: center; width: 80px; } /* Sexo */
            .tabela-jogos th:nth-child(4), .tabela-jogos td:nth-child(4) { text-align: center; width: 140px; white-space: nowrap; } /* Telefone */
            
            .tabela-full-wrapper { position: relative; left: 50%; transform: translateX(-50%); width: 100vw; max-width: 1600px; padding: 0; margin: 0; }
            .tabela-scroll { overflow-x: auto; padding-bottom: 12px; position: relative; }
            .tabela-full-wrapper .tabela-jogos { width: 100%; }
            .tabela-jogos thead th { position: sticky; top: 0; z-index: 5; }

            .btn-export { 
                background-color: #27ae60; color: white; padding: 8px 16px; border: none; 
                border-radius: 5px; cursor: pointer; font-weight: bold; font-size: 0.9rem; margin-top: 10px;
            }
            .btn-export:hover { background-color: #219150; }

            @media (max-width: 600px) { .opcoes-grid { grid-template-columns: 1fr; } }
        `;
...
Joãozinho (Filho)"></textarea>
    ...
    carregarMinhaInscricao(form, statusEl).catch(e => console.error("Erro load:", e));
        ...
        btnCancelar.addEventListener("click", async () => {
            ...
            try {
                ...
                if (r && r.ok) {
                    ...
                    document.getElementById("jogos-resumo-inscricao").style.display = "none";
                    
                    const containerLista = document.querySelector(".jogos-container-lista");
                    if (containerLista) renderizarLista(containerLista);
                } else {
    ...
    form.addEventListener("submit", async (e) => {
        ...
        if (!form.querySelector("#interesse").checked) {
            statusEl.textContent = "⚠️ Marque a caixa 'Desejo integrar a delegação'.";
            statusEl.style.color = "#c0392b";
        ...
        if (!sexo) {
            statusEl.textContent = "⚠️ Informe seu Sexo.";
            statusEl.style.color = "#c0392b";
...
async function carregarMinhaInscricao(form, statusEl) {
    try {
        ...
        form.querySelector("#interesse").checked = true;
        form.querySelector("#sexo").value = data.sexo || "";
        form.querySelector("#qtd_familiares").value = String(data.qtd_familiares || "0");
        form.querySelector("#familiares").value = data.familiares || "";
        form.querySelector("#obs").value = data.observacoes || "";
        ...
        if (resumo) {
            const listaMods = mods.length ? mods.join(", ") : "Nenhuma.";
            const listaFam = (data.familiares || "").trim() ? data.familiares.replace(/\n/g, "<br>") : "Sem familiares.";
            resumo.innerHTML = `
                <div style="margin-bottom:6px;"><strong>Situação Atual:</strong> Inscrito</div>
                <div style="margin-top:6px;"><strong>Modalidades:</strong><br>${listaMods}</div>
                <div style="margin-top:6px;"><strong>Familiares (${data.qtd_familiares || 0}):</strong><br>${listaFam}</div>
            `;
...
// 🟢 NOVAS FUNÇÕES: Idade, Ordenação e Exportação

// 1. Calcular Idade 2026
function calcularIdade2026(dataNascimento) {
    if (!dataNascimento) return "-";
    const anoNasc = new Date(dataNascimento).getFullYear();
    if (isNaN(anoNasc)) return "-";
    return 2026 - anoNasc;
}

// 2. Estado global da ordenação
let direcaoOrdenacao = {};

function ordenarTabela(indiceColuna) {
    const tabela = document.querySelector(".tabela-jogos");
    if (!tabela) return;
    const tbody = tabela.querySelector("tbody");
    const linhas = Array.from(tbody.querySelectorAll("tr"));
    
    const atualDir = direcaoOrdenacao[indiceColuna] || 'asc';
    const novaDir = atualDir === 'asc' ? 'desc' : 'asc';
    direcaoOrdenacao[indiceColuna] = novaDir;

    // Atualiza classes visuais
    tabela.querySelectorAll("th").forEach(th => th.className = "");
    const thAlvo = tabela.querySelectorAll("th")[indiceColuna];
    if (thAlvo) thAlvo.classList.add(novaDir === 'asc' ? 'sort-asc' : 'sort-desc');

    linhas.sort((a, b) => {
        const celulaA = a.children[indiceColuna].innerText.trim().toLowerCase();
        const celulaB = b.children[indiceColuna].innerText.trim().toLowerCase();

        const numA = parseFloat(celulaA);
        const numB = parseFloat(celulaB);

        let comp = 0;
        if (!isNaN(numA) && !isNaN(numB)) comp = numA - numB;
        else comp = celulaA.localeCompare(celulaB);

        return novaDir === 'asc' ? comp : comp * -1;
    });
...
function exportarTabelaCSV() {
    ...
    linhas.forEach(linha => {
        ...
        cols.forEach(col => {
            ...
            if (texto.search(/("|,|\n)/g) >= 0) texto = `"${texto}"`; // Envolve em aspas se necessário
            linhaCsv.push(texto);
        });
        csvData.push(linhaCsv.join(";")); // Ponto e vírgula para Excel PT-BR
    });
...
async function renderizarLista(container) {
    ...
    try {
        ...
        if (!r || !r.ok) { container.innerHTML = "<p style='color:#f88;'>Erro ao carregar.</p>"; return; }
        
        const data = await r.json();
        const lista = Array.isArray(data.inscricoes) ? data.inscricoes : (Array.isArray(data) ? data : []);
        ...
        container.innerHTML = `
            <div class="tabela-full-wrapper">
                <div style="background:#003366; color:#fff; padding:16px; border-radius:10px; margin-bottom:20px; border:1px solid var(--amarelo); text-align:center;">
                    <div style="font-size:1.1rem; font-weight:600;">
                        Inscritos: <strong>${totalTit}</strong> titulares &nbsp;+&nbsp; <strong>${totalFam}</strong> familiares
                    </div>
                    <div style="margin-top:10px; padding-top:10px; border-top:1px solid rgba(255,255,255,0.3); color:var(--amarelo); font-size:1.3rem;">
                        <strong>Total Geral: ${totalTit + totalFam} Pessoas</strong>
                    </div>
                    <button id="btn-export-csv" class="btn-export">📊 Exportar Excel (CSV)</button>
                </div>

                <div class="tabela-scroll">
                    <table class="tabela-jogos" id="tabela-inscritos">
                        <colgroup>
                            <col style="min-width: 180px;"> <col style="width: 60px;">
                            <col style="width: 90px;"> <col style="width: 130px;">
                            <col style="width: auto;"> <col style="width: 60px;">
                            <col style="min-width: 200px;"> <col style="min-width: 200px;">
                        </colgroup>
                        <thead>
                            <tr>
                                <th onclick="window.ordenarJogos(0)">Nome</th>
                                <th onclick="window.ordenarJogos(1)">Idade</th>
                                <th onclick="window.ordenarJogos(2)">Sexo</th>
                                <th onclick="window.ordenarJogos(3)">Telefone</th>
                                <th onclick="window.ordenarJogos(4)">Modalidades</th>
                                <th onclick="window.ordenarJogos(5)">Fam.</th>
                                <th onclick="window.ordenarJogos(6)">Familiares</th>
                                <th onclick="window.ordenarJogos(7)">Obs.</th>
                            </tr>
                        </thead>
                        <tbody>${rows}</tbody>
                    </table>
        ...
        // Atribui ao window para o onclick funcionar
        window.ordenarJogos = ordenarTabela;
        document.getElementById("btn-export-csv").addEventListener("click", exportarTabelaCSV);
