...
2. **Isolamento do App Mobile**:
   ...
   - **Dependências**: O caminho da dependência `@sinprfes/shared-format` foi atualizado para apontar para `file:../backend/shared/format`, garantindo que o app use o código compartilhado de dentro da estrutura FENAPRF.
...
4. **Vínculo com Firebase (Google Services)**:
   - O arquivo `google-services.json` atual é do SINPRF-ES. Será necessário gerar um novo no Firebase Console para o pacote `br.org.fenaprf.app` e substituí-lo.
