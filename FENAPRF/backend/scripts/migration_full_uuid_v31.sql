-- FENAPRF MIGRATION V31: TOTAL UUID RESET (CLEAN SLATE)
-- Objective: Standardize all IDs and Foreign Keys to UUID.
-- Strategy: DROP domain tables and RECREATE with UUID.
...
-- USERS
CREATE TABLE users (
    id UUID PRIMARY KEY, -- No default here, generated in backend (UUIDv7)
    ...
    desarquivado_por UUID REFERENCES users(id),
...
CREATE TABLE assembleia_checkins (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    assembleia_quorum_id UUID REFERENCES assembleia_quoruns(id) ON DELETE CASCADE,
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,
    origem VARCHAR(50),
    registrado_em TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
CREATE TABLE assembleia_mesa (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    assembleia_id UUID REFERENCES assembleias(id) ON DELETE CASCADE,
    presidente_user_id UUID REFERENCES users(id),
    vice_presidente_user_id UUID REFERENCES users(id),
    secretario_user_id UUID REFERENCES users(id),
    secretario_2_user_id UUID REFERENCES users(id),
    definida_por_user_id UUID REFERENCES users(id),
...
CREATE TABLE assembleia_votacoes (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    assembleia_id UUID REFERENCES assembleias(id) ON DELETE CASCADE,
    quorum_snapshot_id UUID REFERENCES assembleia_quoruns(id),
    titulo VARCHAR(255) NOT NULL,
    descricao TEXT,
    status VARCHAR(20) DEFAULT 'ATIVA',
    duracao_segundos INTEGER DEFAULT 60,
    iniciada_por_user_id UUID REFERENCES users(id),
    aberta_em TIMESTAMP WITH TIME ZONE,
