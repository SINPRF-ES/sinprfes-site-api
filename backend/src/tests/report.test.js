...
// Mock auth
let mockUser = { id: 1, perfil_acesso: 'DIRETORIA' };
...
describe('Assembleia Report API', () => {
  beforeEach(() => {
    jest.resetAllMocks();
    mockUser = { id: 1, perfil_acesso: 'DIRETORIA' };

    // Default mocks
    pdfService.gerarPdfRelatorioAssembleia.mockResolvedValue(Buffer.from('pdf-content'));
    filiadosService.buscarPorId.mockResolvedValue({ id: 1, nome: 'Test User', email1: 'test@test.com' });
  });

  test('should allow DIRETORIA to generate report during EM_CURSO', async () => {
    pool.query.mockResolvedValue({ rows: [{ id: VALID_UUID, estado: 'EM_CURSO', titulo: 'Ass 1' }] });

    const response = await request(app).post(`/api/assembleias/${VALID_UUID}/relatorio`);
  ...
  test('should allow FILIADO to generate report during EM_CURSO', async () => {
    mockUser = { id: 2, perfil_acesso: 'FILIADO' };
    pool.query.mockResolvedValue({ rows: [{ id: VALID_UUID, estado: 'EM_CURSO', titulo: 'Ass 1' }] });
  ...
  test('should allow FILIADO to generate report when ENCERRADA', async () => {
    mockUser = { id: 2, perfil_acesso: 'FILIADO' };
    pool.query.mockResolvedValue({ rows: [{ id: VALID_UUID, estado: 'ENCERRADA', titulo: 'Ass 1' }] });
  ...
  test('should block COMUNICADOR from generating report even when ENCERRADA', async () => {
    mockUser = { id: 3, perfil_acesso: 'COMUNICADOR' };
    pool.query.mockResolvedValue({ rows: [{ id: VALID_UUID, estado: 'ENCERRADA', titulo: 'Ass 1' }] });
