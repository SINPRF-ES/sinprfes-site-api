...
async function gerarPublicRefInforme(client, informe) {
  ...
  const prefix = `${datePart}-aniversario-`;
...
exports.listar = async (req, res) => {
  ...
  const method = "GET";
  const endpoint = "/api/aniversarios";
  ...
  try {
    ...
    if (status && typeof status !== 'string') {
      ...
      log.warn("ANIVERSARIOS_GET_INVALID_PARAMS", { requestId, atorId, queryParams, receivedStatusType: typeof status, receivedStatusValue: receivedValue.substring(0, 500) });
    ...
    if (paginacaoPublica) {
      ...
      params.push(ANIVERSARIOS_POR_PAGINA);
      query += ` LIMIT $${params.length}`;
      params.push((paginaAtual - 1) * ANIVERSARIOS_POR_PAGINA);
      query += ` OFFSET $${params.length}`;
    }

    let { rows: aniversarios } = await pool.query(query, params);

    const informesSemRef = aniversarios.filter((i) => i.status === "PUBLICADA" && !i.public_ref);
    ...
    if (aniversarios.length > 0) {
      const ids = aniversarios.map(i => i.id);
      const { rows: allMidias } = await pool.query(
        "SELECT * FROM aniversario_midias WHERE aniversario_id = ANY($1) ORDER BY ordem ASC",
        [ids]
      );

      const midiasMap = new Map();
      allMidias.forEach(m => {
        if (!midiasMap.has(m.aniversario_id)) {
          midiasMap.set(m.aniversario_id, []);
        }
        midiasMap.get(m.aniversario_id).push(m);
      });

      aniversarios.forEach(i => {
    ...
    log.error("ANIVERSARIOS_GET_FAILED", { endpoint, method, requestId, atorId, profile, queryParams, durationMs: Date.now() - start, errorMessage: err.message });
    return res.status(500).json({ success: false, message: "Erro interno ao processar aniversarios.", requestId });
  }
};
...
exports.detalhar = async (req, res) => {
  ...
  const method = "GET";
  const endpoint = `/api/aniversarios/${id}`;
  const atorId = req.user?.id;
  const profile = req.user?.perfil_acesso;
  ...
  try {
    ...
    const { rows: midiaRows } = await pool.query(
      "SELECT * FROM aniversario_midias WHERE aniversario_id = $1 ORDER BY ordem ASC",
      [id]
    );
...
exports.detalharPorRef = async (req, res) => {
  ...
  const method = "GET";
  const endpoint = `/api/aniversarios/ref/${publicRef}`;
  const atorId = req.user?.id;
  const profile = req.user?.perfil_acesso;
  ...
  try {
    ...
    try {
      ...
      const { rows: atuais } = await client.query(
        `SELECT id FROM aniversarios WHERE status_editorial = 'ATUAL' FOR UPDATE`
      );
      ...
      const { rows: createdRows } = await client.query(
        `INSERT INTO aniversarios (titulo, subtitulo, conteudo, status, autor_id, capa_url, destaque, data_informe, status_editorial, is_editable, sort_date)
...
exports.publicar = async (req, res) => {
  ...
  const method = "POST";
  const endpoint = `/api/aniversarios/${id}/publicar`;
  const atorId = req.user?.id;
  if (!atorId) return res.status(401).json({ success: false, message: "Não autenticado", requestId });
  const profile = req.user?.perfil_acesso;
...
exports.arquivar = async (req, res) => {
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT id, status_editorial FROM aniversarios WHERE id = $1 FOR UPDATE`,
      [id]
    );
    ...
    await client.query("COMMIT");
    log.info("ANIVERSARIOS_ARCHIVE_SUCCESS", { requestId, atorId, id });
...
exports.adicionarMidia = async (req, res) => {
  ...
  const method = "POST";
  const endpoint = `/api/aniversarios/${id}/midias`;
  const atorId = req.user?.id;
  if (!atorId) return res.status(401).json({ success: false, message: "Não autenticado", requestId });
  const profile = req.user?.perfil_acesso;
  ...
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO aniversario_midias (aniversario_id, tipo, url, ordem)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [id, tipo || (resourceType === "video" ? "VIDEO" : "IMAGEM"), result.secure_url, ordem || 0]
    );

    log.info("ANIVERSARIOS_ADD_MEDIA_SUCCESS", { endpoint, method, requestId, atorId, profile, id, durationMs: Date.now() - start });
    return res.status(201).json({ ...rows[0], is_capa: false, requestId });
  } catch (err) {
    log.error("ANIVERSARIOS_ADD_MEDIA_FAILED", { endpoint, method, requestId, atorId, profile, id, errorMessage: err.message });
...
exports.removerMidia = async (req, res) => {
  ...
  const method = "DELETE";
  const endpoint = `/api/aniversarios/midias/${midiaId}`;
  const atorId = req.user?.id;
  if (!atorId) return res.status(401).json({ success: false, message: "Não autenticado", requestId });
  const profile = req.user?.perfil_acesso;
  ...
  try {
    const { rows: midiaRows } = await pool.query(
      "SELECT i.id as aniversario_id, i.status_editorial, i.is_editable, i.capa_midia_id FROM aniversarios i JOIN aniversario_midias im ON i.id = im.aniversario_id WHERE im.id = $1",
      [midiaId]
    );
    ...
    if (midiaRows.length > 0 && midiaRows[0].capa_midia_id === midiaId) {
      await pool.query("UPDATE aniversarios SET capa_midia_id = NULL, capa_url = NULL WHERE id = $1", [midiaRows[0].aniversario_id]);
    }

    const { rowCount } = await pool.query("DELETE FROM aniversario_midias WHERE id = $1", [midiaId]);

    if (rowCount === 0) {
      return res.status(404).json({ success: false, message: "Mídia não encontrada.", requestId });
    }

    log.info("ANIVERSARIOS_REMOVE_MEDIA_SUCCESS", { endpoint, method, requestId, atorId, profile, midiaId, durationMs: Date.now() - start });
    return res.json({ success: true, requestId });
  } catch (err) {
    log.error("ANIVERSARIOS_REMOVE_MEDIA_FAILED", { endpoint, method, requestId, atorId, profile, midiaId, errorMessage: err.message });
...
exports.adicionarMidiaExterna = async (req, res) => {
  ...
  const method = "POST";
  const endpoint = `/api/aniversarios/${id}/midias_external`;
  const atorId = req.user?.id;
  if (!atorId) return res.status(401).json({ success: false, message: "Não autenticado", requestId });
  const profile = req.user?.perfil_acesso;
  ...
  try {
    ...
    log.info("ANIVERSARIOS_ADD_EXTERNAL_MEDIA_SUCCESS", { endpoint, method, requestId, atorId, profile, id, durationMs: Date.now() - start });
    return res.status(201).json({ ...rows[0], is_capa: false, requestId });
  } catch (err) {
    log.error("ANIVERSARIOS_ADD_EXTERNAL_MEDIA_FAILED", { endpoint, method, requestId, atorId, profile, id, errorMessage: err.message });
...
exports.obterAssinaturaUpload = async (req, res) => {
  const start = Date.now();
  const method = "POST";
  const endpoint = "/api/aniversarios/upload-signature";
  const requestId = req.requestId || uuidv4();
  const atorId = req.user?.id;
  if (!atorId) return res.status(401).json({ success: false, message: "Não autenticado", requestId });
  const profile = req.user?.perfil_acesso;
  ...
  try {
    ...
    log.info("ANIVERSARIOS_GET_SIGNATURE_SUCCESS", { endpoint, method, requestId, atorId, profile, durationMs: Date.now() - start });
    return res.json({ ...signatureData, requestId });
  } catch (err) {
    log.error("ANIVERSARIOS_GET_SIGNATURE_FAILED", { endpoint, method, requestId, atorId, profile, durationMs: Date.now() - start, errorMessage: err.message });
...
exports.definirCapa = async (req, res) => {
  ...
  try {
    ...
    if (!coverMediaId) {
      await pool.query("UPDATE aniversarios SET capa_midia_id = NULL, capa_url = NULL WHERE id = $1", [id]);
      return res.json({ success: true, capa_midia_id: null, capa_url: null, requestId });
    }

    const { rows: midiaRows } = await pool.query(
      "SELECT id, url, tipo FROM aniversario_midias WHERE id = $1 AND aniversario_id = $2",
      [coverMediaId, id]
    );
    if (midiaRows.length === 0) {
    ...
    await pool.query(
      "UPDATE aniversarios SET capa_midia_id = $1, capa_url = $2 WHERE id = $3",
      [coverMediaId, midiaRows[0].url, id]
    );

    log.info("ANIVERSARIOS_SET_COVER_SUCCESS", { requestId, atorId, id, coverMediaId });
