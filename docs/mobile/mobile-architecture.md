...
4. **Resolution**: Metro is configured to resolve dependencies prioritizing `mobile/node_modules`.
...
- `mobile/metro.config.js`: Metro bundler configuration optimized for monorepo and dependency isolation.

## Build and Update Strategy
- **OTA Updates**: Handled via EAS Update for JS/Asset changes.
- **Native Builds**: Handled via EAS Build for changes requiring native code modification or SDK updates.
