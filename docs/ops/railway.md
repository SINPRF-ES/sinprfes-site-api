...
*   **Playwright (Consulta Processual/TRF1)**: o pacote npm está em `backend/package.json` e o Chromium é preparado explicitamente no build via `build:railway` com `playwright install chromium`; antes disso, o script `backend/scripts/prepare-playwright.js` instala libs Linux via `apt-get` (controlado por `PLAYWRIGHT_INSTALL_SYSTEM_DEPS`, padrão `true`) para evitar falha de libs ausentes (`libglib-2.0.so.0`, etc.).
